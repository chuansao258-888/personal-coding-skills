# Personal Coding Skills Engineering Standards

This document is the canonical code-quality, architecture, consistency, and
verification standard for Personal Coding Skills. Plans, implementations, reviews, and
...
- [Evidence](#evidence)
- [Repository Language And Format Profile](#repository-language-and-format-profile)
- [Mandatory Decision Gates](#mandatory-decision-gates)

## Project Profile

- Purpose: publish six reusable Codex skills with deterministic validation,
  safe Windows linking, and bilingual user documentation.
- Primary languages and formats: Markdown, YAML, JSON, Python 3, and PowerShell.
- Required validation commands: `python scripts/validate_skills.py`,
  `.\scripts\validate-skills.ps1`, and `git diff --check`.
- Shared or local policy: this standard is tracked and applies to contributors,
  local Codex work, and CI-reviewed changes.
...
## Repository Language And Format Profile

### LANG-PY-01 Python Validators Stay Deterministic

- Use the Python standard library unless an approved requirement justifies a
  dependency.
- Use `pathlib.Path` for paths and explicit UTF-8 for text.
- Keep validation functions small, named by the invariant they check, and
  collect actionable failures before returning a non-zero exit code.
- Use type hints for public functions and non-obvious data structures.
- Preserve the dependency-free Linux/Windows CI contract.

### LANG-PS-01 PowerShell Scripts Fail Safely

- Set `$ErrorActionPreference = 'Stop'` for repository automation.
- Use `-LiteralPath` for user or computed paths and resolve targets before
  moving or linking directories.
- Check native command exit codes and convert failures into actionable errors.
- Back up an existing skill before replacement and restore it if link creation
  fails.
- Do not hide destructive operations in pipelines or cross-shell commands.

### LANG-MD-01 Markdown Is Navigable And Bilingual Where Required

- Use stable headings, relative repository links, balanced fences, and concise
  code blocks.
- Keep the Chinese and English README sections semantically synchronized in the
  same change.
- Use Mermaid for maintainable workflow diagrams; use generated bitmaps only
  when they materially improve comprehension and can be stored in the repo.
- Do not add auxiliary README or changelog files inside individual skills.

### LANG-DATA-01 Manifest And Workflow Files Have One Authority

- `skills-manifest.json` is the only publication allowlist.
- Skill names, manifest paths, directory names, frontmatter, and UI metadata
  must agree.
- YAML and JSON remain UTF-8, use stable ordering, and avoid duplicated
  configuration authority.
- GitHub Actions use least-privilege permissions and current supported action
  majors verified from their official repositories.

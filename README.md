...
本仓库发布六个彼此协作的个人 Codex 技能，共同构成一套具有
明确入口、证据、状态和交接契约的工程循环：
...
这项技能用于首次初始化仓库，以及后续维护仓库规范。它不会自动设置
远程仓库、推送代码或重写 Git 全局配置。输出包括英文工程规范、指向该规范的
...
- 按可独立验证的垂直切片拆分实现，避免按技术层一次性铺开全部后端或前端。
...
- 对评测与 benchmark 分开生产链和评测链，核实数据集、judge/evaluator、指标
  ...
  “原报告证据链完整”分开结论；
- 追溯 `pass/completed/inserted/matched` 等字段的生产公式、真实回执或代理值、
  false/missing/failure 数量及消费它的硬门禁，据此判断证据强度；
...
This repository publishes six cooperating personal Codex skills. Together they
form an evidence-based engineering loop with explicit entry gates, states,
artifacts, review ownership, and handoffs:
...
#### Workflow Chart: See the Engineering Loop First
...
from the English template, tailors language rules, and verifies stable IDs. For
existing standards, it preserves current content and merges only missing rules.
It tracks shared/public policy by default, audits local AI files, and makes
`AGENTS.md` point to the canonical standard. It does not configure a remote,
push, or rewrite global Git settings.
...
user-visible output, and operational signals, keeping internal callbacks distinct
from final success. It also recalculates retry/repair counts, derived
...
semantics. Framework conventions provide context but do not establish current
behavior. Current-state claims require independent confirmation from reachable
...
Existing plans, implementation records, findings, and ADRs provide discovery,
original-intent, and history inputs. Reachable implementation evidence remains
authoritative for current state. The skill independently searches beyond their
cited files and verifies reachability, configuration binding, migrations, tests,
cleanup, and runtime evidence. It reconciles each document claim as consistent,
partially implemented, drifted, plan-only, unverified, or omitted from
documentation, and reports historical or expected behavior separately from
current code when they conflict.
If no related plan or implementation record exists, it records the search scope
and continues with full code verification.

For interviews, it generates complete answers. Core
...
uncovered cases. MIME/extension checks, regexes, probes, and prompt constraints
remain limited signals unless stronger evidence supports the claim. Comments,
test names, and parameter names are checked against actual guards, callbacks,
loops, sleeps, artifact timestamps, and assertion boundaries so configured
delays are not misreported as observed sampling intervals. E2E, preflight,
health, and live probe fields must be backed by a real action and acknowledgement;
fixed placeholders, exposure-only checks, unavailable-allowed assertions, or missing
...
- [Matt Pocock's Grilling skill](https://github.com/mattpocock/skills/tree/main/skills/productivity/grilling)

...
|---|---|---|---|
| [`agent-guidelines-init`](skills/agent-guidelines-init/SKILL.md) | 初始化 Git、审计 AI 编码忽略规则、建立仓库级 `AGENTS.md` | 仓库工作规范、知识/陷阱台账、分支与 checkpoint 政策 | `feature-planner` |
...
| [`module-introduction`](skills/module-introduction/SKILL.md) | 基于代码证据编写完整中文模块介绍，追踪架构、参数来源和特殊实现 | 模块 README/架构说明/面试文档、Mermaid 或必要图片 | 文档评审或交付 |
...
- 审计 `.agents/`、`.codex/`、`AGENTS.md`、`CONTEXT.md`、
  `ENGINEERING_STANDARDS.md` 等本地 AI 文件的忽略覆盖；
...
推送代码或重写 Git 全局配置。输出是仓库级指导和可验证的 gitignore 状态。

示例：

```text
使用 $agent-guidelines-init 初始化当前仓库的 Agent 工作规范，审计本地
AI 文件的 .gitignore 覆盖，并建立计划评审、阶段验收和 checkpoint 规则。
```
...
- 已实现模块需要 onboarding、架构 walkthrough、模块 README 或面试说明；
- 需要解释特殊代码机制、参数设定来源、运行链路和故障语义；
...
输出通常是模块介绍、架构说明、参数来源表、验证证据和面试问答。

示例：

```text
使用 $module-introduction 为 memory 模块编写完整中文介绍。
覆盖整体架构、参数来源、关键代码实现、数据与并发语义、验证和限制；
必要时生成并嵌入仓库本地架构图。
```
...
|---|---|---|---|
| [`agent-guidelines-init`](skills/agent-guidelines-init/SKILL.md) | Initialize Git, audit AI-coding ignore rules, and create repository guidance | `AGENTS.md`, knowledge ledger, branch/checkpoint policy | `feature-planner` |
...
| [`module-introduction`](skills/module-introduction/SKILL.md) | Produce a complete Chinese module walkthrough from code evidence | Module README, architecture/parameter/code analysis, interview guide | Documentation review or delivery |

### 3. Detailed Skill Reference

#### 3.1 `agent-guidelines-init`

Use this initializer when a repository lacks durable agent guidance, needs a
Git or AI-ignore audit, or must define branch, review-closure, documentation,
knowledge-ledger, and accepted-phase checkpoint rules.

It resolves the real repository root before initializing Git, audits local
AI-coding files without untracking shared files, discovers actual project
commands and conventions, and creates a repository-specific `AGENTS.md`. It is
not a daily workflow skill and does not configure a remote, push, or rewrite
global Git settings.

```text
Use $agent-guidelines-init to audit this repository, create durable AGENTS.md
guidance, verify local AI-file ignore coverage, and define branch, review, and
accepted-phase checkpoint gates.
```
...
architecture, README, interview, or deep implementation documentation. It
...
architecture, design, parameter provenance, special code paths, data and
concurrency semantics, verification, limitations, and extension points.
```
...
skill mention, relative-link integrity, UTF-8 text, trailing-whitespace rules,
and absence of the maintainer's private machine paths. GitHub Actions runs the
same checks on pushes to `main` and on pull requests.
